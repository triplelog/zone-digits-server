...
outFile = './ne_10m_populated_places_simple/citiesVS9.geojson'
fj = open('./ne_10m_populated_places_simple/citiesVS8.geojson')
...
for i in range(0,leng):
	...
	for ii in range(i+1,leng):
		...
		if d < 100:
			...
			if ratio > math.sqrt(gdp)/math.sqrt(1500000):
				toDelete[oid2]=True
			pairs50+=1
print(pairs50)
td = 0
...
for key in toDelete:
	td += 1
	tdarr.append(key)
tdarr.sort(reverse=True)
for i in range(0,len(tdarr)):
	del cityData['features'][tdarr[i]]
print(td,len(tdarr))		
